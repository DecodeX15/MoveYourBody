  ...
  group('AiInferenceViewModel Tests', () {
    ...
    setUp(() {
      ...
      container = ProviderContainer(
        overrides: [
          aiModelRepositoryProvider.overrideWithValue(mockRepo),
        ],
      );
    });
    ...
    test('initializeAiAndTags should call initModel and set state to ready', () async {
    ...
    test('processUserQuery should find the best match successfully', () async {
      ...
      when(() => mockRepo.generateEmbedding('I want to lose weight'))
      ...
      verify(() => mockRepo.generateEmbedding('I want to lose weight')).called(1);
    });
    ...
    test('processUserQuery should handle null embedding gracefully', () async {
      ...
      when(() => mockRepo.generateEmbedding('invalid')).thenAnswer((_) async => null);
    ...
    test('processUserQuery should catch exceptions from repository', () async {
      ...
      when(() => mockRepo.generateEmbedding('error_trigger'))
          .thenThrow(Exception('Simulated Crash'));

      await notifier.processUserQuery(query: 'error_trigger', targetEmbeddingsDb: {});
